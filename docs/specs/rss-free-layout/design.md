...
| 常量 | 值 | 定义位置 | 含义 / 取值依据 |
|------|-----|---------|----------------|
| `MIN_RATIO` / `MAX_RATIO` | 0.4f / 3.0f | Calculator | 极端比例钳制区间（AD-09 已裁决策略 X）。依据：`0.4` ≈ 1:2.5 长图、`3.0` ≈ 3:1 全景；超出即裁边（B4）。**v1.2 恢复初值**：v1.1 曾收紧 `[0.9, 2.2]` 治行数跳变，用户否决（裁图违背自由样式初衷），行数均衡改由 `itemsPerRow` 统一张数方案承担。**回退路径**：若实测裁切过激可放宽至 `4.0`，或改走策略 Y |
...
| `SHRINK_TOLERANCE` | — | Calculator | **v1.2 已删除**：旧贪心收行阈值，随「整表统一每行张数」方案移除（收行不再动态判定） |
| `TARGET_ROW_HEIGHT_RATIO` | 0.42f | Calculator | 目标行高 = 该比例 × 屏幕宽。**实测校准值**（360px 宽 / 间距 4px / 文字块 46px）：4:3 横图 2 张/行 134px、16:9 宽图 2 张/行 100px、1:1 方图 2 张/行 178px、3:4 竖图 3 张/行 156px。**v1.2 起该值仅用于反推整表统一每行张数 `itemsPerRow`**（`k = availableWidth/(targetRowHeight×avgRatio)`），不再做贪心收行阈值；同类源整表行数一致，消除"横图行 2 张、竖图行 4 张"跳变（用户铁证）。不取 0.33 是因实测该值下混合行高仅约 98px，缩略图辨识度不足 |
...
- **Superseded-by**: 无
- **ChangeLog**: v1.0 初版（自曝初稿取舍缺陷）→ 同日用户裁决定为策略 X → **同日 v1.1 收紧钳制区间 `[0.4, 3.0]` → `[0.9, 2.2]`**（治"横图行 2 张、竖图行 4 张"跳变）→ **同日 v1.2 回滚钳制至 `[0.4, 3.0]` 并重构分行算法**：用户否决 v1.1（"图片会裁剪，做自由样式的目的不就没了"），行数均衡改由**整表统一每行张数 `itemsPerRow`**（按比例自均值反推 k=2~4，贪心收行/回退修正/SHRINK_TOLERANCE 一并移除）承担——行内宽度仍按原图比例分配，常规图零裁剪，仅极端长图/全景（1:2.5 以外）按策略 X 裁边
...
| 5 | 视频角标：自绘矢量 `ic_rss_free_video_badge.xml` | **v1.2 已移除**（drawable 与 item 布局角标视图一并删除） | 2026-09-11 用户裁决——其余 5 种样式右上角均无播放角标，自由布局独加破坏全站一致性；订阅源列表点击即进播放路由，无需角标区分 |
...
| `app/src/main/java/io/legado/app/ui/rss/article/RssArticlesAdapter5.kt` | 新增 | 自由布局 Adapter：绑定图片（`CENTER_CROP`）、标题/时间文字块、**已读态用图片 alpha 降亮**（见实施校准 #2）、`contentDescription` 取标题、tag 防复用错位。**v1.2：视频角标绑定已删** |
| `app/src/main/res/layout/item_rss_article_free.xml` | 新增 | **动态高度容器沿用瀑布流的 `CardView(cardCornerRadius=12dp, clipToOutline, cardElevation=0dp)` 方案**（因格子高度动态，`FilletImageView` 需固定尺寸）；内含图片区 `ImageView(centerCrop, layout_weight=1)` + 图下固定文字块（标题 `maxLines=1` + 时间 `maxLines=1`，高 = `@dimen/rss_free_text_block_height`）。**v1.2：右上角视频角标视图已删** |
...
| `app/src/main/res/drawable/ic_rss_free_video_badge.xml` | ~~新增~~ **v1.2 已删除** | 视频角标矢量图随角标功能一并移除（用户裁决：与其余 5 种样式不一致） |
